 ...
 */
public class DMLSelectParserTest extends AbstractSyntaxTest {
    ...
    public static void main(String[] ars) throws Exception {
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            SQLExprParser exprParser = new SQLExprParser(lexer);
            DMLSelectParser parser = new DMLSelectParser(lexer, exprParser);
    ...
    public void testSelectUnion() throws SQLSyntaxErrorException {
        ...
        SQLLexer lexer = new SQLLexer(sql);
        DMLSelectParser parser = new DMLSelectParser(lexer, new SQLExprParser(lexer));
        ...
        Assert.assertEquals("(SELECT id FROM t1) UNION ALL (SELECT id FROM t2) UNION ALL (SELECT id FROM t3) ORDER BY d DESC LIMIT ?, 1",
                            output);
        ...
        parser = new DMLSelectParser(lexer, new SQLExprParser(lexer));
        ...
        Assert.assertEquals("(SELECT id FROM t1) UNION (SELECT id FROM t2 ORDER BY id) UNION ALL (SELECT id FROM t3) ORDER BY d",
                            output);
    ...
    public void testSelect() throws SQLSyntaxErrorException {
        ...
        Assert.assertNotNull(select);
        output = output2MySQL(select, sql);
        Assert.assertEquals("SELECT * FROM offer AS A STRAIGHT_JOIN wp_image AS B USE KEY FOR JOIN (t1, t2) ON a.member_id = b.member_id INNER JOIN product_visit AS C WHERE a.member_id = c.member_id AND c.member_id = 'abc'",
                            output);
        ...
        Assert.assertNotNull(select);
        output = output2MySQL(select, sql);
        Assert.assertEquals("SELECT DISTINCTROW HIGH_PRIORITY SQL_SMALL_RESULT tb1.id, tb2.id FROM tb1, tb2 WHERE tb1.id2 = tb2.id2",
                            output);
