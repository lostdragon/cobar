...
public class LiteralHexadecimalTest extends TestCase {
    public void testUtf8() throws Exception {
        ...
        LiteralHexadecimal hex = (LiteralHexadecimal) new SQLExprParser(new SQLLexer(sql), "utf-8").expression();
        Assert.assertEquals("邱硕'\\s", hex.evaluation(null));

        sql = "x'd0A'";
        hex = (LiteralHexadecimal) new SQLExprParser(new SQLLexer(sql), "utf-8").expression();
        Assert.assertEquals("\r\n", hex.evaluation(null));
    }
