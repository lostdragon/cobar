 ...
 */
public class DMLInsertParserTest extends AbstractSyntaxTest {
    ...
    public void testInsert() throws SQLSyntaxErrorException {
        ...
        DMLInsertParser parser = new DMLInsertParser(lexer, new SQLExprParser(lexer));
        ...
        parser = new DMLInsertParser(lexer, new SQLExprParser(lexer));
        insert = parser.insert();
        parser.match(Token.EOF);
        output = output2MySQL(insert, sql);
        Assert.assertEquals("INSERT IGNORE INTO test.t1 (t1.id1) VALUES (?) ON DUPLICATE KEY UPDATE ex.col1 = ?, col2 = 12",
                            output);
        ...
        parser.match(Token.EOF);
        output = output2MySQL(insert, sql);
        Assert.assertEquals("INSERT LOW_PRIORITY INTO t1 SELECT id FROM t1 ON DUPLICATE KEY UPDATE ex.col1 = ?, col2 = 12",
                            output);
        ...
        parser.match(Token.EOF);
        output = output2MySQL(insert, sql);
        Assert.assertEquals("INSERT LOW_PRIORITY INTO t1 (col1, t1.col2) VALUES (123, '123\\'4') ON DUPLICATE KEY UPDATE ex.col1 = ?",
                            output);
        ...
        parser.match(Token.EOF);
        output = output2MySQL(insert, sql);
        Assert.assertEquals("INSERT LOW_PRIORITY INTO t1 (col1, t1.col2) SELECT id FROM t3 ON DUPLICATE KEY UPDATE ex.col1 = ?",
                            output);
