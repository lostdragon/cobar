 ...
 */
public final class Dimension2PartitionFunction extends FunctionExpression {
    ...
    private static int convertType(String keyType) {
        if ("long".equalsIgnoreCase(keyType)) return PARTITION_KEY_TYPE_LONG;
        if ("string".equalsIgnoreCase(keyType)) return PARTITION_KEY_TYPE_STRING;
    ...
    private Integer getByXY(int x, int y) {
        if (x >= xSize || y >= ySize) throw new IllegalArgumentException("x, y out of bound: x=" + x + ", y=" + y);
        return x + xSize * y;
    }
     ...
     */
    private static Integer calculate(Object eval,
                                     PartitionUtil partitionUtil,
                                     int keyType,
                                     int hashSliceStart,
                                     int hashSliceEnd) {
        if (eval == UNEVALUATABLE || eval == null) return null;
        switch (keyType) {
    ...
    public FunctionExpression constructFunction(List<Expression> arguments) {
        if (arguments == null || arguments.size() != 2)
            throw new IllegalArgumentException("function "
                                               + getFunctionName()
                                               + " must have 2 arguments but is "
                                               + arguments);
        Dimension2PartitionFunction rst = new Dimension2PartitionFunction(functionName, arguments);
        rst.countX = countX;
        rst.xSize = xSize;
        rst.lengthX = lengthX;
    ...
    //        Dimension2PartitionFunction func = new Dimension2PartitionFunction("test999", new ArrayList<Expression>(2));
