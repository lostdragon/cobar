 ...
 */
public class ServerRouteTestSkip extends AbstractAliasConvert {
    ...
    public void testRouteInsertShort() throws Exception {
        ...
        Assert.assertEquals((int) RouteResultsetNode.DEFAULT_REPLICA_INDEX, rrs.getNodes()[0].getReplicaIndex());
        Assert.assertEquals("offer_dn[44]", rrs.getNodes()[0].getName());
        Assert.assertEquals("insert low_priority into offer set offer_id=123,  group_id=234,gmt=now() on duplicate key update `dual`=1",
                            rrs.getNodes()[0].getStatement());
        ...
        Assert.assertEquals((int) RouteResultsetNode.DEFAULT_REPLICA_INDEX, rrs.getNodes()[0].getReplicaIndex());
        Assert.assertEquals("offer_dn[12]", rrs.getNodes()[0].getName());
        Assert.assertEquals("delete from offer.*,wp_image.* using offer a,wp_image b where a.member_id=b.member_id and a.member_id='abc' ",
                            rrs.getNodes()[0].getStatement());
    ...
    public void testRoute() throws Exception {
        ...
        for (int i = 0; i < 128; i++) {
            ...
            Assert.assertEquals("select * from  offer A where a.member_id='abc' union select * from product_visit b where B.offer_id =123",
        ...
        Assert.assertEquals((int) RouteResultsetNode.DEFAULT_REPLICA_INDEX, rrs.getNodes()[0].getReplicaIndex());
        Assert.assertEquals("offer_dn[44]", rrs.getNodes()[0].getName());
        Assert.assertEquals("UPDATE offer AS "
                                    + aliasConvert("a")
        ...
        Assert.assertEquals((int) RouteResultsetNode.DEFAULT_REPLICA_INDEX, rrs.getNodes()[0].getReplicaIndex());
        Assert.assertEquals("offer_dn[123]", rrs.getNodes()[0].getName());
        Assert.assertEquals("SELECT * FROM wp_image AS "
                                    + aliasConvert("wB")
                                    + " INNER JOIN offer AS "
                                    + aliasConvert("o")
    ...
    public void testGroupLimit() throws Exception {
        ...
        rrs = ServerRouter.route(schema, sql, null, null);
        Assert.assertEquals(-1L, rrs.getLimitSize());
        if (rrs.getNodes().length > 1) Assert.assertEquals(RouteResultset.SUM_FLAG, rrs.getFlag());
        Map<String, RouteResultsetNode> nodeMap = getNodeMap(rrs, 2);
        ...
        rrs = ServerRouter.route(schema, sql, null, null);
        if (rrs.getNodes().length > 1) Assert.assertEquals(RouteResultset.MIN_FLAG, rrs.getFlag());
        if (rrs.getNodes().length > 1) Assert.assertEquals(99L, rrs.getLimitSize());
        nodeMap = getNodeMap(rrs, 2);
        nameAsserter = new NodeNameAsserter("offer_dn[123]", "offer_dn[10]");
        nameAsserter.assertRouteNodeNames(nodeMap.keySet());
        sqlAsserter = new SimpleSQLAsserter();
        ...
        rrs = ServerRouter.route(schema, sql, null, null);
        if (rrs.getNodes().length > 1) Assert.assertEquals(RouteResultset.MAX_FLAG, rrs.getFlag());
        if (rrs.getNodes().length > 1) Assert.assertEquals(99L, rrs.getLimitSize());
        nodeMap = getNodeMap(rrs, 3);
        nameAsserter = new NodeNameAsserter("offer_dn[123]", "offer_dn[10]", "offer_dn[66]");
        nameAsserter.assertRouteNodeNames(nodeMap.keySet());
    ...
    public void testCobarHint() throws Exception {
        ...
        RouteNodeAsserter asserter = new RouteNodeAsserter(nameAsserter, sqlAsserter, new ReplicaAsserter() {
            @Override
            public void assertReplica(Integer nodeIndex, Integer replica) {
                if (nodeIndex.equals(2)) Assert.assertEquals(2, replica.intValue());
                else Assert.assertEquals(RouteResultsetNode.DEFAULT_REPLICA_INDEX, replica);
            }
        });
        for (RouteResultsetNode node : nodeMap.values()) {
            asserter.assertNode(node);
        ...
        asserter = new RouteNodeAsserter(nameAsserter, sqlAsserter, new ReplicaAsserter() {
            ...
            public void assertReplica(Integer nodeIndex, Integer replica) {
                if (nodeIndex.equals(2)) Assert.assertEquals(2, replica.intValue());
                else Assert.assertEquals(1, replica.intValue());
            }
        });
        for (RouteResultsetNode node : nodeMap.values()) {
            asserter.assertNode(node);
    ...
    public void testIgnoreSchema() throws Exception {
        ...
        rrs = ServerRouter.route(schema, sql, null, null);
        Assert.assertEquals("SELECT * FROM ignoreSchemaTest2.offer AS "
                            + aliasConvert("a")
                            + ", offer AS "
                            + aliasConvert("b")
                            + " WHERE ignoreSchemaTest2.offer.offer_id = 1", rrs.getNodes()[0].getStatement());
    }
