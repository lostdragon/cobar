...
public class ServerRoutePerformance {
    ...
    private static class ShardingDefaultSpace extends TestProvider {
        ...
        public void route(SchemaConfig schema, int loop, String sql) throws Exception {
            for (int i = 0; i < loop; ++i) {
                ...
                //                DMLSelectStatement select = new DMLSelectParser(lexer, new SQLExprParser(lexer)).select();
                //                PartitionKeyVisitor visitor = new PartitionKeyVisitor(schema.getTablesSpace());
    ...
    private static class InsertLong extends TestProvider {
        ...
        public void route(SchemaConfig schema, int loop, String sql) throws Exception {
            for (int i = 0; i < loop; ++i) {
                SQLLexer lexer = new SQLLexer(sql);
                DMLInsertStatement insert = new DMLInsertParser(lexer, new SQLExprParser(lexer)).insert();
                //                PartitionKeyVisitor visitor = new PartitionKeyVisitor(schema.getTablesSpace());
                //                insert.accept(visitor);
                //                visitor.getColumnValue();
                //                                SQLLexer lexer = new SQLLexer(sql);
                //                                new DMLInsertParser(lexer, new SQLExprParser(lexer)).insert();
    ...
    private static class InsertLongSQLGen extends TestProvider {
        ...
        public String getSql() throws Exception {
            ...
            SQLLexer lexer = new SQLLexer(sql);
            insert = new DMLInsertParser(lexer, new SQLExprParser(lexer)).insert();
            return sql;
        }
