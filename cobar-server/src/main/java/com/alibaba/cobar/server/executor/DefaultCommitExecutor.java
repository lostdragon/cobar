...
public class DefaultCommitExecutor extends NodeExecutor {
     ...
     * 提交事务
     */
    public void commit(final OkPacket packet, final ServerSession session, final int initCount) {
        // 初始化
        final ReentrantLock lock = this.lock;
        lock.lock();
        try {
        ...
        for (RouteResultsetNode rrn : target.keySet()) {
            if (rrn == null) {
                try {
                    getLogger().error("null is contained in RoutResultsetNodes, source = "
                                      + session.getSource()
                                      + ", bindChannel = "
                                      + target);
                } catch (Exception e) {
    ...
    private void _commit(MySQLChannel mc, ServerSession session) {
    ...
    private void handleException(Channel mc, ServerSession session, Exception e) {
        isFail.set(true);
        if (decrementCountBy(1)) {
            try {
                session.clear();
