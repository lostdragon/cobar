...
public final class SingleNodeExecutor extends NodeExecutor {
     ...
     * 单数据节点执行
     */
    public void execute(RouteResultsetNode rrn, ServerSession ss, int flag) {
        // 初始化
        final ReentrantLock lock = this.lock;
        lock.lock();
        try {
     ...
     * 已绑定数据通道的执行
     */
    private void bindingExecute(final RouteResultsetNode rrn, final ServerSession ss, final Channel c, final int flag) {
     ...
     * 新数据通道的执行
     */
    private void newExecute(final RouteResultsetNode rrn, final ServerSession ss, final int flag) {
     ...
     * 数据通道执行
     */
    private void execute0(RouteResultsetNode rrn, ServerSession ss, Channel c, int flag) {
        final ServerConnection sc = ss.getSource();
     ...
     * 处理结果集数据
     */
    private void handleResultSet(RouteResultsetNode rrn, ServerSession ss, MySQLChannel mc, BinaryPacket bin, int flag)
            throws IOException {
        final ServerConnection sc = ss.getSource();
     ...
     * 处理RowData数据
     */
    private void handleRowData(RouteResultsetNode rrn, ServerSession ss, MySQLChannel mc, ByteBuffer bb, byte id)
            throws IOException {
        final ServerConnection sc = ss.getSource();
        this.packetId = id;
     ...
     * 下一个数据接收任务
     */
    private void handleNext(final RouteResultsetNode rrn,
                            final ServerSession ss,
                            final MySQLChannel mc,
                            final ByteBuffer bb,
                            final byte id) {
        final ServerConnection sc = ss.getSource();
        sc.getProcessor().getExecutor().execute(new Runnable() {
     ...
     * 执行异常处理
     */
    private void handleError(int errno, String message, ServerSession ss) {
        endRunning();
