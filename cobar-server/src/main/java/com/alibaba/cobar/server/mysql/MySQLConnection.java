...
import com.alibaba.cobar.server.UnknownTxIsolationException;
 ...
 * @author xianmao.hexm
 */
public final class MySQLConnection extends BackendConnection {
    ...
    public static class StatusSync {
        ...
        public void execute() throws UnsupportedEncodingException {
            ...
            packet.packetId = 0;
            packet.command = Commands.COM_QUERY;
        ...
        private static CommandPacket getCharsetCommand(int ci) {
            ...
            cmd.packetId = 0;
            cmd.command = Commands.COM_QUERY;
    ...
    public void quit() {
        if (!isQuit.getAndSet(true) && !isClosed()) {
            if (isAuthenticated) {
                //QS_TODO check
    ...
    public void handleError(int errCode, Throwable t) {
        ...
        default:
            ...
            if (handler instanceof MySQLHandler) {
                ((MySQLHandler) handler).connectionError(t);
            }
        }
    }
    ...
    public boolean setResponseHandler(ResponseHandler queryHandler) {
        if (handler instanceof MySQLHandler) {
            ((MySQLHandler) handler).setResponseHandler(queryHandler);
            return true;
        }
        return false;
    }
