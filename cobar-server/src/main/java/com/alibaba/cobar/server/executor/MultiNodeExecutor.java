 ...
 */
package com.alibaba.cobar.server.executor;
...
import com.alibaba.cobar.net.packet.MySQLPacket;
...
import com.alibaba.cobar.server.UnknownDataNodeException;
import com.alibaba.cobar.server.node.Channel;
import com.alibaba.cobar.server.node.MySQLChannel;
...
public final class MultiNodeExecutor extends NodeExecutor {
     ...
     */
    public void execute(RouteResultsetNode[] nodes, final boolean autocommit, final ServerSession ss, final int flag) {
        // 初始化
        final ReentrantLock lock = this.lock;
        lock.lock();
        try {
     ...
     * 新通道的执行
     */
    private void newExecute(final RouteResultsetNode rrn,
                            final boolean autocommit,
                            final ServerSession ss,
                            final int flag) {
        ...
        if (dn == null) {
            handleFailure(ss, rrn, new SimpleErrInfo(
                    new UnknownDataNodeException("Unknown dataNode '" + rrn.getName() + "'"),
     ...
     * 执行
     */
    private void execute0(RouteResultsetNode rrn, Channel c, boolean autocommit, ServerSession ss, int flag) {
     ...
     * 处理RowData数据
     */
    private void handleRowData(final RouteResultsetNode rrn, Channel c, ServerSession ss) throws IOException {
     ...
     * 处理下一个任务
     */
    private void handleNext(final RouteResultsetNode rrn, final Channel c, final ServerSession ss) {
        final ServerConnection sc = ss.getSource();
        sc.getProcessor().getExecutor().execute(new Runnable() {
            @Override
            public void run() {
     ...
     */
    private void handleSuccessEOF(ServerSession ss, BinaryPacket bin) {
     ...
     * @throws nothing never throws any exception
     */
    private void handleSuccessOK(ServerSession ss, RouteResultsetNode rrn, boolean autocommit, OkPacket ok) {
        if (decrementCountAndIsZero()) {
            if (isFail.get()) {
                notifyFailure(ss);
                return;
    ...
    private void handleFailure(ServerSession ss, RouteResultsetNode rrn, ErrInfo errInfo) {
        try {
            // 标记为执行失败，并记录第一次异常信息。
     ...
     * @throws nothing never throws any exception
     */
    private void notifyFailure(ServerSession ss) {
        try {
            // 清理
            ss.clear();
